                ...
                <table class="table table-hover table-bordered">
                    <tr>
                        <td colspan="2" class="col-md-12">
                            <select name="category" id="category">
                                ...
                                <option value="chair" <c:if test="${product.category == 'chair'}">selected</c:if>>의자
                                </option>
                                <option value="desk" <c:if test="${product.category == 'desk'}">selected</c:if>>책상
                                </option>
                                <option value="bed" <c:if test="${product.category == 'bed'}">selected</c:if>>침대
                                </option>
                                <option value="sofa" <c:if test="${product.category == 'sofa'}">selected</c:if>>소파
                                </option>
                                <option value="etc" <c:if test="${product.category == 'etc'}">selected</c:if>>기타
                                </option>
                            </select>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <form
                                    method="post"
                                    action="/admin/products/imageUpload"
                            ...
                            <c:if test="${!empty images}">
                                ...
                                <c:forEach var="image" items="${images}" varStatus="a">
                                    <img class="d-block vh-100 mt-5" src="/files/${image.dbFileName}">
                                    삭제 : <input type="checkbox" name="deleteImageCheck"
                                                value="${image.productImageId}">${image.originalFileName}
                                </c:forEach>
                            </c:if>
                        </td>
                    </tr>
